...
This document defines the externally observable contract of PCW-MCP `0.2.0-beta.3`. It preserves the frozen beta.2 contract and adds safe workstream creation; it is not a `1.0` stability claim or a published release.
...
- the 13 registered MCP tools, schemas, descriptions, success payloads, and documented error semantics;
...
`package.json` is the single software-version source of truth. The MCP server and `--version` derive `0.2.0-beta.3` from it. Software version and `pcw.yml.version` are separate: the latter is an optional string or number under the current configuration contract.

Beta.1 and beta.2 are frozen by annotated tags. Beta.3 is not tagged or published by this development block.
...
The beta.3 baseline contains exactly 13 tools:

`create_workstream`, `get_project_info`, `list_workstreams`, `get_workstream_info`, `get_continuity`, `list_shared_context`, `list_sources`, `get_inventory`, `search_inventory`, `read_text_source`, `read_docx_source`, `read_pdf_source`, and `update_continuity`.

The POC-only `hello` tool was intentionally removed before the beta freeze. MCP initialization and tool discovery provide protocol-level availability checks. The intended write surface is limited to `create_workstream` for structural creation and `update_continuity` for optimistic-concurrency checkpoint replacement. There are no delete, rename, archive, or arbitrary-file write tools.
...
The `pcw.yml` schema is unchanged for this beta. Dynamic logical names remain supported, physical paths remain user-defined, and continuity may be configured without specialized workstream context. `create_workstream` uses a safe generated path convention only for newly automated entries; manual configurations may retain custom in-root layouts. Tightening the optional string-or-number `version` field requires a future explicit schema migration.
...
The owner selected a proprietary private-beta model. Package metadata declares `UNLICENSED` with `private: true`; no open-source license or redistribution right is granted. The English and Spanish private-beta terms permit only authorized private evaluation. Nothing in this block publishes the package, transmits the handoff kit, or creates a beta.3 release tag.
